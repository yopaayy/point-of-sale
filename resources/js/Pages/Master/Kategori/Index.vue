...
watch(params, debounce(function (value) {
    router.get(route('master.kategori.index'), value, { preserveState: true, replace: true });
}, 300), { deep: true });
...
const submitForm = () => {
    if (isEditing.value) {
        form.put(route('master.kategori.update', editingId.value), {
            onSuccess: () => closeDialog(),
        });
    } else {
        form.post(route('master.kategori.store'), {
            onSuccess: () => closeDialog(),
        });
    }
};
...
const deleteItem = () => {
    form.delete(route('master.kategori.destroy', itemToDelete.value.id), {
        onSuccess: () => {
        ...
        <div class="py-12 bg-gray-50 min-h-screen">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow-sm sm:rounded-xl overflow-hidden border border-gray-100">
                    ...
                    <div class="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:justify-between sm:items-center bg-white space-y-3 sm:space-y-0">
                        ...
                        <div class="flex items-center space-x-3">
                            <div class="relative">
                                ...
                                <input v-model="params.search" type="text" placeholder="Cari kategori..." class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 sm:text-sm transition duration-150 ease-in-out" />
                            </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50/50">
                                <tr>
                                    ...
                                    <SortableHeader label="Nama Kategori" field="nama" :params="params" @sort="handleSort" />
                                    <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-32">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-100">
                                <tr v-for="item in kategori.data" :key="item.id" class="hover:bg-gray-50/50 transition-colors">
                    ...
                    <!-- Pagination -->
                    <div class="bg-white px-4 py-3 border-t border-gray-100 sm:px-6 flex items-center justify-between" v-if="kategori.links && kategori.data.length > 0">
                        ...
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-gray-700">
                                    Menampilkan <span class="font-medium">{{ kategori.from }}</span> sampai <span class="font-medium">{{ kategori.to }}</span> dari <span class="font-medium">{{ kategori.total }}</span> hasil
                                </p>
                            </div>
                            <div>
                                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                    <component 
                                        :is="link.url ? 'a' : 'span'"
                                        v-for="(link, k) in kategori.links" 
                                        :key="k" 
                                        :href="link.url"
                                        v-html="link.label"
                                        :class="[
                                            ...
                                            k === 0 ? 'rounded-l-md' : '',
                                            k === kategori.links.length - 1 ? 'rounded-r-md' : ''
                                        ]"
        ...
        <Modal :show="isDeleteModalOpen" @close="isDeleteModalOpen = false" max-width="sm">
            <div class="px-6 py-4">
                ...
                <div class="mt-4 text-sm text-gray-600">
                    Apakah Anda yakin ingin menghapus kategori <b>{{ itemToDelete?.nama }}</b>? Tindakan ini tidak dapat dibatalkan.
                </div>
