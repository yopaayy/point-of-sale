...
watch(params, debounce(function (value) {
    router.get(route('master.satuan.index'), value, { preserveState: true, replace: true });
}, 300), { deep: true });
...
const openEditModal = (item) => {
    ...
    form.nama = item.nama;
    form.nama_pendek = item.nama_pendek;
    form.clearErrors();
    isModalOpen.value = true;
};
...
const submitForm = () => {
    if (isEditing.value) {
        form.put(route('master.satuan.update', editingId.value), {
            onSuccess: () => closeDialog(),
        });
    } else {
        form.post(route('master.satuan.store'), {
            onSuccess: () => closeDialog(),
        });
    }
};
...
const deleteItem = () => {
    form.delete(route('master.satuan.destroy', itemToDelete.value.id), {
        onSuccess: () => {
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow-sm sm:rounded-xl overflow-hidden border border-gray-100">
                    ...
                    <div class="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:justify-between sm:items-center bg-white space-y-3 sm:space-y-0">
                        <h3 class="text-lg font-semibold text-gray-800">Daftar Satuan (Base Unit)</h3>
                        <div class="flex items-center space-x-3">
                            <div class="relative">
                                ...
                                <input v-model="params.search" type="text" placeholder="Cari satuan..." class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 sm:text-sm transition duration-150 ease-in-out" />
                            </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50/50">
                                <tr>
                                    ...
                                    <SortableHeader label="Nama Satuan" field="nama" :params="params" @sort="handleSort" />
                                    <SortableHeader label="Nama Pendek (Singkatan)" field="nama_pendek" :params="params" @sort="handleSort" />
                                    <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-32">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-100">
                                <tr v-for="item in satuan.data" :key="item.id" class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.id }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ item.nama }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.nama_pendek || '-' }}</td>
                    ...
                    <!-- Pagination -->
                    <div class="bg-white px-4 py-3 border-t border-gray-100 sm:px-6 flex items-center justify-between" v-if="satuan.links && satuan.data.length > 0">
                        <div class="flex-1 flex justify-between sm:hidden">
                        </div>
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-gray-700">
                                    Menampilkan <span class="font-medium">{{ satuan.from }}</span> sampai <span class="font-medium">{{ satuan.to }}</span> dari <span class="font-medium">{{ satuan.total }}</span> hasil
                                </p>
                            </div>
                            <div>
                                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                    <component 
                                        ...
                                        :class="[
                                            ...
                                            k === 0 ? 'rounded-l-md' : '',
                                            k === satuan.links.length - 1 ? 'rounded-r-md' : ''
                                        ]"
            ...
            <div class="px-6 py-4">
                ...
                <div class="mt-4">
                    <form @submit.prevent="submitForm">
                        <div class="mb-4">
                            <InputLabel for="nama" value="Nama Satuan" />
                            <TextInput
                                id="nama"
                                ref="namaInput"
                                v-model="form.nama"
                        ...
                        <div class="mb-4">
                            <InputLabel for="nama_pendek" value="Singkatan (Opsional)" />
                            <TextInput
                                id="nama_pendek"
                                v-model="form.nama_pendek"
                                type="text"
                                class="mt-1 block w-full"
                                placeholder="Contoh: Pcs"
                            />
                            <InputError :message="form.errors.nama_pendek" class="mt-2" />
                        </div>
        ...
        <Modal :show="isDeleteModalOpen" @close="isDeleteModalOpen = false" max-width="sm">
            <div class="px-6 py-4">
                ...
                <div class="mt-4 text-sm text-gray-600">
                    Apakah Anda yakin ingin menghapus satuan <b>{{ itemToDelete?.nama }}</b>? Tindakan ini tidak dapat dibatalkan.
                </div>
